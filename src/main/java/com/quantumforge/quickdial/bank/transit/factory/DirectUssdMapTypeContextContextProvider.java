...
public class DirectUssdMapTypeContextContextProvider implements UssdMappingContextProvider {
    ...
    public UssdExecutionContext getMatchingContext(String mapping, List<UssdExecutionContext> contexts) {
        return contexts
                .stream()
                .filter(ussdExecutionContext -> ussdExecutionContext.getUssdMapping().equalsIgnoreCase(mapping))
                .findFirst()
                .orElse(getParameterizedMatchingContext(mapping, contexts, quickDialUtil));
    }

    @Override
    public boolean supportUssdMapType(UssdMapType mapType) {
        return mapType == UssdMapType.DIRECT_MAPPING;
    }
}
